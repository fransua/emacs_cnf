...
in the current *Python* session."
    ...
    (let* ((ugly-return nil)
	   ...
	   (line (replace-regexp-in-string "\'" "\"" line)) ;;; hack francois
           (pattern (buffer-substring-no-properties beg end))
           (completions nil)
           (completion-table nil)
           completion
      ...
      (setq completion-table (loop for str in completions
                                   collect (list str nil)))
	  ;;; hack francois
      (setq div (ceiling (+ (/ (length completion-table) 16) 0.5)))
      (setq outnum
	    (string-to-number (replace-regexp-in-string "\\]:.*" "" 
				     (replace-regexp-in-string ".*In \\[" "" (replace-regexp-in-string "\n" "" ugly-return )))))
      (if (> (length completion-table) 16)
	  (setq number (+ 0 (* (mod outnum div) 16))))
      (if (> (length completion-table) 16)
	  (setq completion-table-new (subseq completion-table number (+ 16 number)))
	(setq completion-table-new completion-table))
      ;;; end hack
      (setq completion (try-completion pattern completion-table))
      (cond ((eq completion t))
            ((null completion)
            ...
            ((not (string= pattern completion))
             ...
             (insert completion))
            (t
                    ;;; little hack francois
					;(with-output-to-temp-buffer "*IPython Completion*"
					;(display-completion-list (all-completions pattern completion-table)))
             (message "%s" (with-output-to-string (display-completion-list (all-completions pattern completion-table-new))))))))
                     ;;; end little hack francois
)
